    ...
    const generateLaTeXCode = () => {
        const header = `\\documentclass[10pt,a4paper]{article}
            ...
            \\begin{center}
                ...
                \\vspace{5pt}
                \\small ${personalInfo.address || 'Address'} \\hfill ${personalInfo.email || 'email'} \\hfill ${personalInfo.phone || 'phone'}
            \\end{center}
        `;
    
        const sectionsLatex = sections.map(section => `
            \\section*{\\textcolor{darkblue}{\\textbf{${htmlToLatex(section.header) || 'Section Title'}}}} % Section header with bluish color and no numbering
            \\noindent ${htmlToLatex(section.description).replace(/\n/g, '\\\\') || 'Section content...'}
            \\vspace{5pt} \\noindent \\rule[0mm]{\\linewidth}{0.5mm} % Line under the heading
        `).join('\n');
